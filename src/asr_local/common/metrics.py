...
def normalize_vi(text: str) -> str:
    """Vietnamese ASR normalization: lowercase, drop punctuation, keep accents."""
    text = text.lower()
...
def wer(refs: Iterable[str], hyps: Iterable[str]) -> float:
    """Corpus WER from word-level Levenshtein distance."""
    total_err = 0
    total_words = 0
...
def extract_text(item) -> str:
    """NeMo transcribe() may return a str or an object with .text."""
